...
def generate_api_classes(
    input_file: Path = typer.Option(
        ..., '--input', '-i', help='Path to the input JSONL file'
    ),
    ...
    for class_name, methods in api_classes.items():
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            ...
            for method in methods:
                ...
                docstring = generate_docstring(
                    {
                        ...
                        'implementation_notes': method.get(
                ...
                for line in lines[
                    1:-1
                ]:  # Skip first/last lines that have the triple quotes
                    f.write(f'        {line}\n')
                f.write('        """\n')
                ...
                if isinstance(params, dict):
                    ...
                    for param_name in params:
                        if param_name not in path_params:
                            ...
                            snake_name = re.sub(
                                r'[^a-zA-Z0-9_]', '_', snake_name
                            )
                            snake_name = re.sub(r'_+', '_', snake_name)
                            f.write(
                                f'            "{param_name}": {snake_name},\n'
                            )
                    f.write('        }\n')
                elif isinstance(params, list):
                    f.write(
                ...
                f.write(
                    '        response = self.api.'
                    + (method.get('http_method', 'post')).lower()
                    + '(\n'
                )
                f.write('            path,\n')
                f.write('            json=params\n')
