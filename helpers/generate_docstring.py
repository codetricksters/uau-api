...
def format_type_hint(param_type: Any) -> str:
    ...
    if isinstance(param_type, dict):
        ...
        return 'bool'
    elif isinstance(param_type, str) and ('T' in param_type or 'Date' in param_type):
        return 'datetime'
    else:
        return 'Any'
    ...
    if parameters:
        ...
        if isinstance(parameters, dict):
            for param_name, param_type in parameters.items():
                param_docs.append(f"    {format_param_description(param_name, param_type)}")
        elif isinstance(parameters, list):
            param_docs.append("    parameters (List[Dict]): List of parameter dictionaries for the request")
        sections.append("\n".join(param_docs))
        
        # Add example parameter structure
        sections.append("Parameter Structure:")
        sections.append(indent(json.dumps(parameters, indent=4, ensure_ascii=False), "    "))
