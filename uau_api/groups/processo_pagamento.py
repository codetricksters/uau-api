...
class ProcessoPagamento:
    ...
    def aprovar_processos(
        self, processos: Optional[List[Dict]] = None
    ) -> requests.Response:
        """
    ...
    def consultar_processos(
        ...
        PROCESSO_KEYS_TO_REMOVE = ['Itens', 'Parametro']
        ...
        if response.headers.get('Content-Type', '').startswith('application/json'):
            ...
            for processo in data:
                for key in PROCESSO_KEYS_TO_REMOVE:
                    processo.pop(key, None)
                for parcela in processo.get('Parcelas', []):
                    for key in PARCELA_KEYS_TO_REMOVE:
                        parcela.pop(key, None)

            response._content = __import__('json').dumps(data).encode()
            return response
        return response
    ...
    def integrar_processo_pagamento_uauws(
        self, xml_proc: Optional[str] = None
    ) -> requests.Response:
        """
